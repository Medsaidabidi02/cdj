...
class Database {
  ...
  async runMigrations(): Promise<void> {
    try {
      ...
      // MySQL 5 doesn't support IF NOT EXISTS for columns, so we try and ignore errors
      try {
        await this.pool.execute(`ALTER TABLE users ADD COLUMN profile_picture VARCHAR(512) NULL`);
      } catch (e) { /* ignore if column exists */ }
      
      try {
        await this.pool.execute(`ALTER TABLE users ADD COLUMN avatar_updated_at TIMESTAMP NULL`);
      } catch (e) { /* ignore if column exists */ }
