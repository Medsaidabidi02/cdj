...
import { getPublicVideoUrl, getPublicAssetUrl, isValidVideoPath, getAllResolutionUrls, AVAILABLE_RESOLUTIONS, DEFAULT_RESOLUTION } from '../services/hetznerService';
import { optionalAuth, AuthRequest } from '../middleware/auth';

const router = Router();

console.log('🎬 Videos API loaded - Hetzner HLS-only mode with multi-resolution support');

/**
 * GET /api/videos
 * Get all videos with subject/course info
 * Returns public HLS URLs for each video with all available resolutions
 */
router.get('/', async (req, res) => {
  try {
    ...
    const videos = result.rows.map(video => {
      ...
      let resolutionUrls: Record<string, string> = {};
      
      if (videoPath && isValidVideoPath(videoPath)) {
        try {
          // Generate default resolution URL (720p)
          publicUrl = getPublicVideoUrl(videoPath, DEFAULT_RESOLUTION);
          // Generate all resolution URLs
          resolutionUrls = getAllResolutionUrls(videoPath);
        } catch (error) {
      ...
      return {
        ...
        resolution_urls: resolutionUrls,
...
router.get('/:id', optionalAuth, async (req: AuthRequest, res) => {
  try {
    ...
    // Generate public HLS URL with resolution support
    const videoPath = video.video_path;
    let publicUrl = null;
    let thumbnailUrl = null;
    let resolutionUrls: Record<string, string> = {};
    ...
    console.log(`✅ Found video ${id} with public HLS URL (${DEFAULT_RESOLUTION}p)`);
    res.json({
      ...video,
      hls_url: publicUrl,
      playback_url: publicUrl,
