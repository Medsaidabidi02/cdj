...
// Upload video metadata (manual upload process)
router.post('/videos', async (req: AuthRequest, res) => {
  try {
    const { title, description, video_path, thumbnail_path, duration, subject_id, file_size, is_active } = req.body;

    if (!title || !video_path || !subject_id) {
      return res.status(400).json({ error: 'Title, video path, and subject ID are required' });
    }

    const result = await database.query(`
      INSERT INTO videos (title, description, video_path, thumbnail_path, duration, subject_id, file_size, is_active, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
    `, [title, description || '', video_path, thumbnail_path || null, duration || 0, subject_id, file_size || 0, is_active !== false]);

    const videoId = result.insertId;

    // Trigger Notifications for course enrollees
    try {
      // 1. Get Course ID
      const subjectResult = await database.query('SELECT course_id, title FROM subjects WHERE id = ?', [subject_id]);
      if (subjectResult.rows.length > 0) {
        const { course_id, title: subjectTitle } = subjectResult.rows[0];
        
        // 2. Get Course info
        const courseResult = await database.query('SELECT title FROM courses WHERE id = ?', [course_id]);
        const courseTitle = courseResult.rows[0]?.title || 'Cours';

        // 3. Get Enrolled Users
        const enrolledUsers = await database.query('SELECT user_id FROM user_courses WHERE course_id = ?', [course_id]);
        const userIds = enrolledUsers.rows.map((u: any) => u.user_id);

        if (userIds.length > 0) {
          notificationService.notifyUsers(userIds, {
            title: `Nouvelle vidéo: ${courseTitle}`,
            message: `Une nouvelle vidéo "${title}" a été ajoutée dans ${subjectTitle}`,
            type: 'video',
            rel_id: course_id // Link to course
          });
        }
      }
    } catch (notifErr) {
      console.error('⚠️ Could not send video notifications:', notifErr);
    }

    res.status(201).json({ success: true, id: videoId });
  } catch (error) {
