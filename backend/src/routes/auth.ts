...
router.post('/login', async (req, res) => {
  try {
    ...
    const userResult = await database.query(
      'SELECT id, name, email, password, is_admin, is_approved, is_teacher, phone, profile_picture, is_logged_in, created_at FROM users WHERE LOWER(TRIM(email)) = ?',
      [email]
    );
...
router.get('/verify', async (req, res) => {
  try {
    ...
    const userResult = await database.query(
      'SELECT id, name, email, phone, profile_picture, is_admin, is_approved, is_teacher FROM users WHERE id = ?',
      [decoded.id]
    );
    ...
    console.log(`✅ Token verified for user ${user.id} | is_teacher=${user.is_teacher} | is_admin=${user.is_admin}`);
    res.json({
      success: true,
      user: {
        id: user.id,
        ...
        is_teacher: !!user.is_teacher,  // Always boolean, never undefined
      }
    });
