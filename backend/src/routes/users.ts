...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { name, email, isAdmin, isApproved, isTeacher } = req.body;

    console.log(`🔄 PUT /api/users/${id} - Updating user for Medsaidabidi02 at 2025-09-09 15:12:54`);
    console.log('📝 Update data:', { name, email, isAdmin, isApproved, isTeacher });
    ...
    if (isTeacher !== undefined)  {
      // is_teacher column added by migration add_live_streaming.sql
      try {
        await database.query('UPDATE users SET is_teacher = ? WHERE id = ?', [isTeacher, id]);
      } catch (colErr: any) {
        console.warn('⚠️ is_teacher column missing – run migration add_live_streaming.sql');
      }
    }
    ...
    // Try to fetch is_teacher too
    let isTeacherValue = false;
    try {
      const tResult = await database.query('SELECT is_teacher FROM users WHERE id = ?', [id]);
      isTeacherValue = !!tResult.rows[0]?.is_teacher;
    } catch { /* column may not exist yet */ }

    console.log(`✅ User ${id} updated by Medsaidabidi02`);
    res.json({ success: true, message: 'User updated', user: { ...result.rows[0], is_teacher: isTeacherValue } });
  } catch (error) {
...
// ─── Toggle teacher role (admin only) ─────────────────────────────────────
router.post('/:id/toggle-teacher', async (req, res) => {
  try {
    const { id } = req.params;
    const { isTeacher } = req.body; // boolean

    console.log(`🎓 POST /api/users/${id}/toggle-teacher - isTeacher=${isTeacher}`);

    try {
      await database.query(
        'UPDATE users SET is_teacher = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
        [isTeacher ? 1 : 0, id]
      );
    } catch (colErr: any) {
      return res.status(500).json({
        success: false,
        message: 'is_teacher column not found. Run migration: backend/migrations/add_live_streaming.sql',
      });
    }

    const result = await database.query(
      'SELECT id, name, email, is_admin, is_approved, is_teacher FROM users WHERE id = ?',
      [id]
    );
    ...
    console.log(`✅ User ${id} teacher role set to ${isTeacher}`);
    res.json({ success: true, message: `Teacher role ${isTeacher ? 'granted' : 'revoked'}`, user: result.rows[0] });
  } catch (error) {
    console.error(`❌ Error toggling teacher role for ${req.params.id}:`, error);
    res.status(500).json({ success: false, message: 'Error updating teacher role' });
  }
});
